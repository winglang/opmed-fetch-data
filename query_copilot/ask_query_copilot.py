...
from query_copilot.constants import (
    TEMPERATURE,
    TOP_P,
    FREQUENCY_PENALTY,
    PRESENCE_PENALTY,
...
def compute_differences(plans):
    ...
    changes_scores = [
        score_block_difference(block, alternative_plan_blocks[block_id])
        for block_id, block in original_plan_blocks.items()
        if block_id in alternative_plan_blocks
    ]
...
def query_lang_model(content):
    ...
    hashed_prompt = json_fingerprint.create(
        input=json.dumps(request | headers | {'url': lang_model_url}), hash_function=hash_functions.SHA256, version=1
    )
