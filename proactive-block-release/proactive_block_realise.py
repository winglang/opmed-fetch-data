...
def get_blocks_status(start, end, tenant):
    ...
    return {
        block['data_id']: {k: v for k, v in block.items() if k in BLOCK_FIELDS_TO_RETURN} for block in response['Items']
    }
...
def proactive_block_realise(event, context):
    ...
    queryStringParameters = {
        key: val for key, val in event.get('queryStringParameters', {}).items() if key in ['from', 'to']
    }
...
if __name__ == '__main__':
    event = {
        'queryStringParameters': {'from': '2024-03-01', 'to': '2024-05-01', 'save_to_s3': True},
        'headers': {
